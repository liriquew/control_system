...
type less func(*entities.NodeWithTask, *entities.NodeWithTask) bool

var lessFuncMap map[int]func(*entities.NodeWithTask, *entities.NodeWithTask) bool

func init() {
	lessFuncMap = make(map[int]func(*entities.NodeWithTask, *entities.NodeWithTask) bool)
	lessFuncMap[1] = func(n1, n2 *entities.NodeWithTask) bool {
		return n1.Node.ID < n2.Node.ID
	}
}

func addPriorityDependencies(graph *entities.GraphWithTasks, nodesValueMap map[int64]float64, lf less) {
	workerTasks := make(map[int64][]*entities.NodeWithTask)

	// Группируем задачи по исполнителям
	for _, node := range graph.Nodes {
		if node.Node.AssignedTo != nil {
			workerID := *node.Node.AssignedTo
			workerTasks[workerID] = append(workerTasks[workerID], node)
		}
	}
	...
	for _, tasks := range workerTasks {
		...
		for i := 0; i < len(tasks)-1; i++ {
			current := tasks[i]
			next := tasks[i+1]
			current.AdditionalDependencies = append(current.AdditionalDependencies, next.Node.ID)
		}
	}
}
...
func FindCriticalPath(graph *entities.GraphWithTasks, nodesValueMap map[int64]float64) ([][]int64, error) {
	if HasCycle(graph) {
	...
	nodeAmountMap := make(map[int64]*nodeAmount, len(graph.Nodes))
	// список смежности
	adjacencyList := make(map[int64][]int64, len(graph.Nodes))
	// обратный список смежности (для обратного хода)
	adjacencyListInversed := make(map[int64][]int64, len(graph.Nodes))

	// построение списков
	for _, node := range graph.Nodes {
		adjacencyList[node.Node.ID] = append(node.Node.DependencyNodeIDs, node.AdditionalDependencies...)

		if _, ok := adjacencyListInversed[node.Node.ID]; !ok {
			adjacencyListInversed[node.Node.ID] = []int64{}
		}
		for _, depNode := range adjacencyList[node.Node.ID] {
			if _, ok := adjacencyListInversed[depNode]; !ok {
				adjacencyListInversed[depNode] = []int64{}
			}

			adjacencyListInversed[depNode] = append(adjacencyListInversed[depNode], node.Node.ID)
		}

		nodeAmountMap[node.Node.ID] = &nodeAmount{
			node: node,
			min:  math.MaxFloat64,
	...
	for _, nodeID := range lastNodes {
		...
		adjacencyList[node.Node.ID] = append(adjacencyList[node.Node.ID], dummyNodeID)
	}

	// straight part
	var queue []int64
	doneNodesSet := make(map[int64]struct{}, len(graph.Nodes))

	for nodeID, inversedNodeDependencies := range adjacencyListInversed {
		if len(inversedNodeDependencies) == 0 {
			queue = append(queue, nodeID)
			nodeAmountMap[nodeID].min = 0
