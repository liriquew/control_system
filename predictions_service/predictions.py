...
class Predicator():
    ...
    def _plot_model_predictions(self, model: GradientBoostingRegressor):
        """Визуализирует предсказания модели"""
        import matplotlib.pyplot as plt
        x_range = np.linspace(1, 72, 100).reshape(-1, 1)
        y_range_pred = model.predict(x_range)

        plt.plot(x_range, y_range_pred, color="orange", label="Gradient Boosting Model", linewidth=2)
        plt.xlabel("Planned Time (X)")
        plt.ylabel("Actual Time (Y)")
        plt.legend()
        plt.grid(True)
